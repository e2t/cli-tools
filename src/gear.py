...
def _ratio_range(base: float, mincoef: float, maxcoef: float) -> str:
...
def main() -> None:
    ...
    def calc_and_print() -> None:
        ...
        print(f"Диаметр впадин зубьев = {module.value * (teeth.value - 2.5):n}")
        ...
        print(f"Ширина зуба = {_ratio_range(module.value, 6, 8)}")
        print(f"Диаметр вала (ГОСТ 6636-69) = {shaft:n}")
        print(f"Диаметр ступицы = {_ratio_range(shaft, 1.5, 2)}")
        print(f"Длина ступицы = {_ratio_range(shaft, 1.2, 1.8)}")
        print(f"Толщина венца = {_ratio_range(module.value, 2.5, 4)}")
        print(f"Толщина диска = {_ratio_range(module.value, 3, 3.5)}")

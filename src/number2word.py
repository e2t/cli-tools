...
s_orders: dict[int, tuple[str, str, str]] = {
    ...
    25: ('кваттуорвигинтиллион', 'кваттуорвигинтиллиона',
    ...
    35: ('кваттуортригинтиллион', 'кваттуортригинтиллиона',
    ...
    38: ('септентригинтиллион', 'септентригинтиллиона',
...
s_units: dict[int, tuple[str, str]] = {
    1: ('один', 'одна'),
    2: ('два', 'две'),
    3: ('три', 'три'),
    4: ('четыре', 'четыре'),
...
def abc2str(abc: int, order: int) -> str:
    assert 0 < abc <= 999
    a, bc = abc // 100, abc % 100
    b, c = bc // 10, bc % 10
    s = []
    if a > 0:
        s.append(s_hundreds[a])
    if 11 <= bc <= 19:
        s.append(s_more_ten[bc])
        index = 2
    else:
        if b > 0:
            s.append(s_decades[b])
        if c > 0:
            s.append(s_units[c][1 if order == 1 else 0])
        if c == 1:
            index = 0
        elif 2 <= c <= 4:
            index = 1
            ...
            index = 2
    if order > 0:
        s.append(s_orders[order][index])
    return ' '.join(s)


MAXLEN = max(s_orders) * 3 + 3
...
def check_input(value: str) -> bool:
    if not value.isdigit():
        ...
        return False
    if len(value) > MAXLEN:
        print(f'Поддерживаются числа меньше 10^{MAXLEN}')
        return False
    return True


def main() -> None:
    number = StrParameter('Целое число (только цифры)', check_input)
